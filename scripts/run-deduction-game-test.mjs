...
要求：凶手唯一，线索公平，每个非凶手都必须有可发现的排除证据，不要超自然、双胞胎、秘密通道。`;

async function callGenerate(stage, currentDraft = {}, userDirection = "", retries = 1) {
...
async function main() {
  ...
  const seed = await callGenerate("gameTruthSeed", {}, "", 0);
  ...
  if (!seed.validation?.valid) {
    await fs.writeFile(
      path.join(outDir, "deduction-game-test-log.md"),
      `# Deduction Game API Test\n\n## Validation Failed\n\n${JSON.stringify(seed.validation, null, 2)}\n`,
      "utf8"
    );
    throw new Error(`Generated case did not pass local rules: ${seed.validation?.issues?.join("; ") || "unknown issue"}`);
  }
  ...
  const firstCharacter =
    deductionCase.characters.find((character) => firstEvidence.relatedCharacterIds?.includes(character.id)) ||
    deductionCase.characters.find((character) => !character.isCulprit) ||
    deductionCase.characters[0];
  ...
  const log = `# Deduction Game API Test
